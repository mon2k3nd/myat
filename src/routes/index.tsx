...
import { CalendarPlus, ChevronLeft, ChevronRight, ChevronUp, Copy, Heart, MapPin, Maximize2, Music2, Navigation, Pause, Share2, Sparkles, X } from "lucide-react";
...
import qrBrideAsset from "@/assets/qr-co-dau.png.asset.json";
import qrGroomAsset from "@/assets/qr-chu-re.png.asset.json";
...
function WeddingInvitation() {
  ...
  return <main className={`paper-texture min-h-screen text-foreground ${opening ? "story-opening" : ""}`}>
    ...
    <section id="gallery" className="gallery-stage py-12 md:py-18"><div className="mx-auto max-w-6xl"><div className="reveal px-6 text-center"><p className="text-[10px] uppercase tracking-[.3em] text-primary">Chương III · Những thước phim</p><h2 className="mt-3 text-[2.6rem] leading-[1.1] md:text-5xl">Một đời, thật nhiều dịu dàng</h2><p className="mx-auto mt-3 text-xs text-muted-foreground">Chạm vào ảnh để xem trọn khoảnh khắc</p></div>
      <div className="mt-8 grid grid-cols-2 items-start gap-1.5 px-1.5 sm:gap-3 sm:px-3 md:grid-cols-3">{gallery.map((src,i) => <Button type="button" variant="image" key={src} onClick={() => setLightboxIndex(i)} className="gallery-tile reveal group relative aspect-[3/4] h-auto w-full min-w-0 overflow-hidden p-0"><img src={src} alt={`Khoảnh khắc cưới ${i+1}`} loading="lazy" className="absolute inset-0 block h-full w-full object-cover object-[50%_25%] transition duration-700 group-hover:scale-[1.035]"/><span className="gallery-sheen absolute inset-0"/><span className="absolute bottom-3 left-3 font-display text-sm italic text-foreground">{String(i+1).padStart(2,"0")}</span></Button>)}</div></div>
    </section>
    ...
    <section id="gifts" className="light-section bg-secondary/25 px-6 py-12 md:py-18"><div className="mx-auto max-w-4xl"><div className="reveal text-center"><p className="text-[10px] uppercase tracking-[.3em] text-primary">Chương VI · Tấm lòng</p><h2 className="mt-3 text-[2.6rem] leading-[1.1] md:text-5xl">Gửi quà mừng</h2><p className="mx-auto mt-4 max-w-md text-sm leading-7 text-muted-foreground">Sự hiện diện của bạn đã là món quà quý giá nhất. Nếu muốn gửi thêm lời chúc phúc, bạn có thể quét mã bên dưới.</p></div><div className="reveal mt-8 grid gap-4 sm:grid-cols-2"><GiftCard side="Cô dâu" name="Tống Thị Thảo My" bank="VietinBank · CN Tây Hà Nội - PGD Trung Hòa" account="104884006027" qr={qrBrideAsset.url}/><GiftCard side="Chú rể" name="Trần Xuân Tú" bank="Vietcombank" account="0491000175392" qr={qrGroomAsset.url}/></div></div></section>

    <section className="relative min-h-[82svh] overflow-hidden bg-foreground"><img src={img9} alt="Thảo My và Xuân Tú dưới tấm voan cưới" className="absolute inset-0 h-full w-full object-cover opacity-60"/><div className="absolute inset-0 bg-gradient-to-t from-foreground via-foreground/20 to-transparent"/><div className="relative mx-auto flex min-h-[82svh] max-w-xl flex-col items-center justify-end px-6 pb-[7.5rem] text-center text-primary-foreground"><Sparkles className="mb-5 text-secondary"/><p className="font-display text-lg italic leading-[2] text-primary-foreground/80">Cảm ơn bạn đã đi hết<br/>câu chuyện của chúng mình.</p><h2 className="mt-6 text-5xl">The End</h2><p className="mt-2 text-xs uppercase tracking-[.28em] text-secondary">See you there</p><p className="mt-9 text-xs leading-6 text-primary-foreground/65">Thảo My & Xuân Tú<br/>03 · 10 · 2026</p></div></section>
...
function FamilyCard({side,father,mother,address,mapsUrl,latLng,placeName}:{side:string;father:string;mother:string;address:string;mapsUrl:string;latLng:string;placeName:string}) {
  const appleUrl = `https://maps.apple.com/?ll=${latLng}&q=${encodeURIComponent(placeName)}`;
  return <article className="rounded-sm border border-border bg-card p-7 text-center"><p className="text-xs uppercase tracking-[.2em] text-primary">{side}</p><h3 className="mt-5 text-2xl">{father}<br/>{mother}</h3><p className="mx-auto mt-4 max-w-sm text-sm leading-6 text-muted-foreground">{address}</p><div className="mt-6 flex justify-center gap-3"><a title="Mở Google Maps" aria-label={`Chỉ đường đến ${side}`} href={mapsUrl} onClick={(e) => openExternal(e, mapsUrl)} target="_blank" rel="noreferrer" className="grid h-11 w-11 place-items-center rounded-full bg-primary text-primary-foreground"><Navigation size={18}/></a><a title="Mở Apple Maps" aria-label={`Mở Apple Maps đến ${side}`} href={appleUrl} onClick={(e) => openExternal(e, appleUrl)} target="_blank" rel="noreferrer" className="grid h-11 w-11 place-items-center rounded-full border border-primary text-primary"><MapPin size={18}/></a></div></article>;
}
function GiftCard({side,name,bank,account,qr}:{side:string;name:string;bank:string;account:string;qr:string}) {
  const [copied, setCopied] = useState(false);
  async function copyAccount() {
    try { await navigator.clipboard.writeText(account); } catch { window.prompt("Sao chép số tài khoản:", account); }
    setCopied(true); window.setTimeout(() => setCopied(false), 2000);
  }
  return <article className="rounded-sm border border-border bg-card p-6 text-center"><p className="text-xs uppercase tracking-[.2em] text-primary">{side}</p><div className="mx-auto mt-5 max-w-64 overflow-hidden rounded-sm border border-border bg-card"><img src={qr} alt={`Mã QR chuyển khoản của ${name}`} loading="lazy" className="block h-auto w-full"/></div><h3 className="mt-5 text-2xl">{name}</h3><p className="mt-1 text-sm text-muted-foreground">{bank}</p><div className="mt-4 flex items-center justify-center gap-2 border-t border-border pt-4"><code className="text-base tracking-wider">{account}</code><button type="button" onClick={copyAccount} aria-label={`Sao chép số tài khoản của ${name}`} className="inline-flex items-center gap-1 rounded-full border border-primary px-3 py-1.5 text-xs font-medium text-primary"><Copy size={13}/>{copied ? "Đã sao chép" : "Sao chép"}</button></div></article>;
}
